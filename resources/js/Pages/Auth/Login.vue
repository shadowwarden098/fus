...
<template>
  <GuestLayout>
    ...
    <!-- Mensaje de estado -->
    <div v-if="status" class="mb-4 text-sm font-medium text-green-400 bg-green-900/30 backdrop-blur-sm border border-green-500/50 rounded-lg p-3">
      {{ status }}
    </div>
    ...
    <form @submit.prevent="submit" class="animated-form">
      ...
      <div class="mt-4 block">
        <label class="flex items-center cursor-pointer group">
          <Checkbox name="remember" v-model:checked="form.remember" class="rounded border-purple-500/50 text-purple-600 focus:ring-purple-500 focus:ring-offset-slate-900" />
          <span class="ms-2 text-sm text-purple-300/80 group-hover:text-purple-200 transition-colors">Recuérdame en las sombras</span>
        </label>
      </div>
      ...
      <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <Link
          ...
          class="text-sm text-purple-400 hover:text-purple-300 underline focus:outline-none transition-colors"
        >
      ...
      <div class="text-center">
        <p class="text-purple-300/70 text-sm">
          ¿Nuevo en Shadow Garden? 
          <Link :href="route('register')" class="text-purple-400 hover:text-purple-300 font-semibold underline transition-colors ml-1">
            Únete ahora
          </Link>
        </p>
      </div>
